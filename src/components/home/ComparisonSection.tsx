...
import type { HomeComparisonData } from "@/types/home";

interface ComparisonSectionProps {
  data: HomeComparisonData | null;
}

export function ComparisonSection({ data }: ComparisonSectionProps) {
  if (!data) return <div className="py-24 text-center text-slate-600">Loading Comparison...</div>;

  const { header, table_rows, feature_cards } = data;
        ...
        <div className="text-center mb-20">
          <h2 className="text-3xl md:text-5xl font-bold mb-4 leading-tight">
            ...
            <span className="block">
               <span className="text-gradient-custom">{header.title_connector}</span>
               <span className="text-white ml-2">{header.title_suffix}</span>
            </span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {feature_cards.map((card) => {
            ...
            return (
              <div 
                key={card.id}
                className={`relative rounded-3xl p-8 md:p-12 overflow-hidden bg-[#050a18] border border-white/5 group transition-all duration-700 shadow-2xl ${
                    isPurple ? 'hover:border-purple-500/30' : 'hover:border-blue-500/30'
                }`}
              >
                {/* Background Glows (根据主题还原) */}
                <div className={`absolute top-0 right-0 w-[300px] h-[300px] rounded-full blur-[80px] transition-all duration-700 pointer-events-none ${
                    isPurple ? 'bg-purple-900/10 group-hover:bg-purple-800/20' : 'bg-blue-900/10 group-hover:bg-blue-800/20'
                }`}></div>
                <div className={`absolute bottom-0 left-0 w-[200px] h-[200px] rounded-full blur-[60px] pointer-events-none ${
                    isPurple ? 'bg-blue-900/10' : 'bg-cyan-900/10'
                }`}></div>
                
                <div className="relative z-10">
                  <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br flex items-center justify-center mb-8 border border-white/5 shadow-inner group-hover:scale-105 transition-transform duration-500 ${
                      isPurple 
                        ? 'from-purple-500/10 to-transparent text-purple-400' 
                        : 'from-blue-500/10 to-transparent text-blue-400'
                  }`}>
                     {/* SVG Icon (根据主题还原) */}
                     {isPurple ? (
                  ...
                  <p className={`text-xs font-mono mb-6 uppercase tracking-widest ${
                      isPurple ? 'text-purple-500/60' : 'text-blue-500/60'
                  }`}>
                    {card.subtitle}
                  </p>
